...
export default function FacultyAttendance() {
  ...
  // 🎯 New: Assignments State (Real data from MySQL)
  ...
  const fetchData = async () => {
    try {
      ...
      const [profRes, historyRes, todayRes, pendingRes, assignRes] = await Promise.allSettled([
        axios.get(`${API_BASE}/faculty/profile`, { withCredentials: true }),
        axios.get(`${API_BASE}/faculty/attendance/history`, { withCredentials: true }),
        axios.get(`${API_BASE}/faculty/attendance/today`, { withCredentials: true }),
        axios.get(`${API_BASE}/attendance/pending`, { headers }),
        axios.get(`${API_BASE}/attendance/assignments`, { headers }) // Gets Subjects/Classes
      ]);
      ...
      if (assignRes.status === 'fulfilled' && assignRes.value.data?.success) setAssignments(assignRes.value.data);
  ...
  const handleVerify = async (recordId, action) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.post(`${API_BASE}/attendance/verify`, { recordId, action }, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleCreateSession = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await axios.post(`${API_BASE}/attendance/session/approve`, sessionForm, {
      ...
      <div className="max-w-8xl mx-auto">
        ...
        <div className="flex gap-2 mb-8 bg-slate-200/50 p-1.5 rounded-2xl w-fit">
          <button 
            ...
            className={`px-8 py-3 rounded-xl font-black text-xs uppercase tracking-widest transition-all flex items-center gap-2 ${viewMode === "personal" ? "bg-blue-600 text-white shadow-sm" : "text-white-500 hover:text-white-700"}`}
          >
          ...
          <button 
            ...
            className={`px-8 py-3 rounded-xl font-black text-xs uppercase tracking-widest transition-all flex items-center gap-2 ${viewMode === "create" ? "bg-blue-600 text-white shadow-md" : "text-white-500 hover:text-white-700"}`}
          >
             ...
             <div className="bg-white rounded-[3rem] overflow-hidden border border-slate-200 shadow-sm">
              <div className="p-10 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center text-left">
                ...
                <div className="bg-blue-50 text-blue-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase">
                  {pendingStudents.length} Requests Pending
                </div>
              </div>
